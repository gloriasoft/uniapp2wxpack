...
function uniRequireWxResource () {
    return $.replace(/[\s\S]*/, function (match, p1) {
        const injectList = ['var __uniPackNativeRequireInject={};\n']
        ...
        match = match.replace(regExpUniRequire, (match, p1) => {
            ...
            const cmd = `__uniPackNativeRequireInject[${injectKey}] = require(${injectKey});\n`
            if (injectList.indexOf(cmd) < 0) {
                injectList.push(cmd)
            }
            return `__uniPackNativeRequireInject[${injectKey}]`
        })
        if (injectList[1]) {
